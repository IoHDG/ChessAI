...
private Stack getPawnMoves(int x, int y, String piece){
  ...
  for(int k=1;k < 3;k++){
    ...
    int tmpy1 = y+k;
      ...
      if(!piecePresent(((tmp1.getXC()*75)), (((tmp1.getYC()*75)+20)))){
        moves.push(validM);
      }
      else{
        if(checkWhiteOponent(((tmp1.getXC()*75)), ((tmp1.getYC()*75)+20))){
          break;
        }
        else{
          break;
...
private Stack getRookMoves(int x, int y, String piece){
  ...
  */
  ...
  }//end of the loop with y increasing and Y doing nothing...
  for(int l=1;l < 8;l++){
    int tmpx4 = x;
    int tmpy4 = y-l;
    if(!(tmpy4 > 7 || tmpy4 < 0)){
